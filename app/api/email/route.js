...
export async function POST(request) {
  ...
  const draftInput = getDraftInputById(draft.input_id);
  ...
  updateDraft(draft.id, { email_sent: true });
  addEmailLog({ user_id: session.uid, draft_id: draft.id, status: "sent", http_status: 200 });
  addActivityLog({
    user_id: session.uid,
